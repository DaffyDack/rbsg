    ...
    <Dialog v-model:visible="visible" modal header="Функционал" :style="{ width: '80%' }">
      <div class="flex">
        <div>
          <div
            class="img"
            :style="{ backgroundImage: 'url(' + imgW.imgUrl + '/' + userInfo.img + ')' }"
          ></div>
        </div>
        <div>
          ...
          <div class="flex items-center mb-8">
            <label for="department" class="font-semibold" :style="{ minWidth: '11.5rem' }"
              >Отдел</label
            >
            <div id="department" class="flex-auto">{{ userInfo.department }}</div>
          </div>
          <div class="flex items-center mb-8">
            <label for="company" class="font-semibold" :style="{ minWidth: '11.5rem' }"
              >Компания</label
            >
            <div id="company" class="flex-auto">{{ userInfo.company }}</div>
          </div>
          <div class="flex items-center mb-8">
            <label for="positions" class="font-semibold" :style="{ minWidth: '11.5rem' }"
              >Должность</label
            >
            <div id="positions" class="flex-auto">{{ userInfo.positions }}</div>
          </div>
          <div class="flex items-center mb-8">
            <label for="mobile" class="font-semibold" :style="{ minWidth: '11.5rem' }"
              >Мобильный</label
            >
            <div id="mobile" class="flex-auto">{{ userInfo.mobilephone }}</div>
          </div>
          <div class="flex items-center mb-8">
            <label for="mobileWork" class="font-semibold" :style="{ minWidth: '11.5rem' }"
              >Мобильный (рабочий)</label
            >
            <div id="mobileWork" class="flex-auto">{{ userInfo.workphone }}</div>
          </div>
          <div class="flex items-center mb-8">
            <label for="datebirth" class="font-semibold" :style="{ minWidth: '11.5rem' }"
              >Дата рождения</label
            >
            <div id="datebirth" class="flex-auto">{{ userInfo.datebirth }}</div>
          </div>
          <div class="flex items-center mb-8">
            <label for="locations" class="font-semibold" :style="{ minWidth: '11.5rem' }"
              >Рабочее место</label
            >
            <div id="locations" class="flex-auto">{{ userInfo.locations }}</div>
          </div>
          <div class="flex items-center mb-8">
            <label for="jobfunctions" class="font-semibold" :style="{ minWidth: '11.5rem' }">
              Долж. Обяз.</label
            >
